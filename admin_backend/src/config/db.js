...
function buildMongoUriFromParts() {
  ...
  if (!username || !password) {
    throw new Error(
      "MongoDB host was provided, but MONGO_USER and MONGO_PASSWORD are missing. Add them to admin_backend/.env or use MONGO_URI."
    );
  }
...
export async function connectDB(uri) {
  ...
  try {
    ...
    if (likelyUriIssue) {
      throw new Error(
        "MongoDB URI looks malformed. If your password contains special characters like @, :, / or #, encode it or use MONGO_HOST + MONGO_USER + MONGO_PASSWORD in admin_backend/.env."
      );
    }
